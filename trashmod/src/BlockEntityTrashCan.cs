    ...
    public class BlockEntityTrashCan : BlockEntityOpenableContainer
    {
        ...
        public void EmptyTrash()
        {
            if (!TrashSlot.Empty)
            {
                ...
                TrashSlot.MarkDirty();
                Api.Logger.Event("[TrashMod] TrashSlot has been emptied.");
            }
        }
        ...
        public override void OnReceivedClientPacket(IPlayer fromPlayer, int packetId, byte[] data)
        {
            ...
            if (packetId == 1004)
            {
                // Move this to Audit when done debugging
                Api?.Logger?.Event("[TrashMod] TrashCan received GUI packed from player. {0} clicked the Empty button.", fromPlayer.PlayerName);
                EmptyTrash();
                MarkDirty();
            }
